...
from .models import MyUser, MyCompany, db, Job, FINANCE_STAGE, FIELD, EDUCATION, EXP
...
class RegisterUserForm(FlaskForm):
    ...
    def validate_email(self, field):
        if MyUser.query.filter_by( email=field.data ).first() or \
                MyCompany.query.filter_by( email=field.data ).first():
...
class UserDetailForm(RegisterUserForm):
    ...
    def validate_email(self, field):
        if current_user.email != self.email.data and \
                MyUser.query.filter_by( email=field.data ).first():
            raise ValidationError('邮箱已被其他账号使用')
...
class JobForm(FlaskForm):
    ...
    # is_enable = SelectField('发布', choices=[('1', '立即发布'), ('0', '暂不发布')])
    submit = SubmitField('提交')
