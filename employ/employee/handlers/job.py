...
def all_jobs():
    ...
    return render_template( 'job/all_jobs.html', active='jobs', jobs=jobs )
...
def job_search():
    ...
    if keyword is not None and keyword != '':
        filter_jobs = Job.query.filter(Job.name.like('%{}%'.format(keyword)))
        count = filter_jobs.count()
        if count != 0:
            return render_template('search.html', filter_jobs=filter_jobs, name='job')
        else:
