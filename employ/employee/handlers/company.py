...
from ..models import MyCompany, Job, Delivery, db
...
def index():
    ...
    pagination = MyCompany.query.filter( *flt ).order_by( MyCompany.updated_at.desc() ).paginate(
...
def all_companies():
    ...
    return render_template( 'company/all_companies.html', active='all_companies', companies=companies )
...
def company_search():
    kw = request.args.get('kw')
    if kw is not None and kw !='':
        filter_companies = MyCompany.query.filter( MyCompany.name.like( '%{}%'.format( kw ) ) )
        count = filter_companies.count()
        if count != 0:
            return render_template('search.html', filter_companies=filter_companies, name='company')
        else:
