...
def loadDataSet():
    dataSet = np.loadtxt(
        "/Users/worlder/Documents/Language/Python3/dataSet.csv")
    return dataSet
...
def minDistance(dataSet, centroidList):
    ...
    for item in dataSet:
        ...
        for i in range(k):
            vec2 = centroidList[i]
            distance = calculateDistance(vec1, vec2)  # error
            if distance < minDis:
                minDis = distance
...
def getVar(centroidList, clusterDict):
    ...
    for key in clusterDict.keys():
        ...
        for item in clusterDict[key]:
            vec2 = item
            distance += calculateDistance(vec1, vec2)
        sum += distance
    return sum
